...
describe("App", () => {
  ...
  it("shows the resume banner when saved data exists", async () => {
    ...
    // Using findByText as the banner might render asynchronously or after effect
    expect(
  ...
  it("resets the saved session when clicking the resume banner reset button", async () => {
    ...
    // It seems there is no confirmation dialog in the current implementation of resetFormData
    // or it's handled differently. Reverting to direct expectation.
    expect(removeItemSpy).toHaveBeenCalledWith("mediation_form_v1");
    await waitFor(() => {
      expect(mockToast.success).toHaveBeenCalledWith("Form data reset successfully");
    });
  });
