...
export const SmartSuggestions = ({
  ...
  // Debounce the value to prevent excessive calculation
  const debouncedValue = useDebounce(currentValue, 500);

  const suggestions = useMemo(() => {
    if (!fieldType || debouncedValue.length <= 2) return [];

    const templates = SUGGESTION_TEMPLATES[fieldType] || [];
    let generated = templates;

    // Filter suggestions based on current value
    const searchTerm = debouncedValue.trim().toLowerCase();
    if (searchTerm) {
      generated = templates.filter(suggestion =>
        suggestion.toLowerCase().includes(searchTerm) ||
        searchTerm.includes(suggestion.toLowerCase())
      );
    }

    // Add context-specific suggestions
    if (context.partyAName && context.partyBName) {
      generated = generated.map(suggestion =>
        suggestion.replace("[person]", context.partyAName || "the other person")
      );
    }

    return generated.slice(0, 5);
  }, [fieldType, debouncedValue, context.partyAName, context.partyBName]);
